...
// СОХРАНЕНИЕ ПОЛЕЙ И ФОРМ
	...
	$OriginalPic="userfiles/picoriginal/".$node["pic"]; $PreViewPic="userfiles/picitem/".$node["pic"]."?r=".time();
	...
	$AdminText.="<input name='picname' type='hidden' value='".$node["pic"]."'>";
		...
		$AdminText.=$C5."<div class='Info'><input type='checkbox' name='exclusive' value='1'>  добавить на фотографию маркировку «Эксклюзив» (отметить при загрузке новой фотографии)</div>";
		$AdminText.=$C5."<div class='Info'><input type='checkbox' name='publishnews' value='1'>  добавить на фотографию маркировку «Народная новость» (отметить при загрузке новой фотографии)</div>";
		$AdminText.=$C5."<div class='Info'><input type='checkbox' name='ownnews' value='1'>  добавить на фотографию ватермарк «ProKazan» (отметить при загрузке новой фотографии)</div>";
		$AdminText.=$C5."<div class='Info'><input type='checkbox' name='goodnewsvz' value='1'>  добавить на фотографию ватермарк «Хорошие новости» (отметить при загрузке новой фотографии)</div>";
	$AdminText.="</div></form>";
	...
	if ($node["pic"]!="") {
        ...
        $xy = array();
        $i = 0;
        $tmp = explode(";", $node["picxy"]);
        foreach ($tmp as $val) {
            $tmp2 = explode("=", $val);
            $xy[$tmp2[0]] = explode(",", $tmp2[1]);
        }
        ...
        foreach ($GLOBAL['AutoPicPaths'] as $path => $size) {
            ...
            if ($sw != 0 && $sh != 0) {
                ...
                $cs = array();
                $cs = $xy[$path];
                list($ow, $oh) = getimagesize("../" . $OriginalPic);
                if(empty($cs)) {
                    $oaspect = $ow / $oh;
                    $naspect = $sw / $sh;
                    if( $oaspect > $naspect ) {
                        $xw = $oh * $naspect;
                        $cs = [intval(($ow - $xw) / 2), 0, intval(($ow + $xw) / 2), $oh];
                    } else {
                        $yh = $ow / $naspect;
                        $cs = [0, intval(($oh - $yh) / 2), $ow, intval(($oh + $yh) / 2)];
                    }
                }
		...
		$AdminText.="<input name='picname' type='hidden' value='".$node["pic"]."'>";
		$AdminText.=$C5."<div class='Info'><input type='checkbox' name='exclusive' value='1'>  добавить на фотографию маркировку «Эксклюзив» (отметить при сохранении старой фотографии)</div>";
		$AdminText.=$C5."<div class='Info'><input type='checkbox' name='publishnews' value='1'>  добавить на фотографию маркировку «Народная новость» (отметить при сохранении старой фотографии)</div>";
		$AdminText.=$C5."<div class='Info'><input type='checkbox' name='ownnews' value='1'>  добавить на фотографию ватермарк «ProKazan» (отметить при сохранении старой фотографии)</div>";
		$AdminText.=$C5."<div class='Info'><input type='checkbox' name='goodnewsvz' value='1'>  добавить на фотографию ватермарк «Хорошие новости» (отметить при сохранении старой фотографии)</div>";
