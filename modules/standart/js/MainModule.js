...
function ShowBadComment(id) {
    $(".ComHiddenPart-" + id).slideToggle('fast');
    $(".BadCommentA-" + id).slideToggle('fast');
}
...
function SendUserComment(form) {
    ...
    JsHttpRequest.query('/modules/standart/UsersComment-JSReq.php', {
        ...
        if (result) { /*s*/
            ...
            $(".CommentSend", form).html('Добавить комментарий');
            obj.html(result["Text"]);
            obj.attr("class", 'CommentMsg ' + result["Class"]);
            if (result["Code"] == 1) {
                $("#UserComAnswer", form).html("");
                $(".UserComAnswerC", form).html("");
                $(".UserComAnswer", form).hide();
                $(".UserComAnswerC", form).hide();
                $("#NoComments").length && $("#NoComments").remove();
...
function BannersSystem() {
    ...
    if (in_array(userid, admins)) {
        $('#Banner-3-1').length && $('#Banner-3-1').parents('.banner-bottom').remove();
        return false;
    }
    JsHttpRequest.query("/advert/getBanner.php", {'domain': domain}, function (result, errors) {
        ...
        if( $('#Banner-3-1').length && ! $('#Banner-3-1').html().trim()) {
            $('#Banner-3-1').parents('.banner-bottom').remove();
        }
    }, true);
}
...
function BannersShow(pid, num, item) {
    var b = item.split(";");
    num = num - (-1);
    if (($("#Banner-" + pid + "-" + num).size() != 0 && b[0] != 0) || pid == 4 || pid == 24) {
        ...
        if (view == 1) {
            var flashvars = (b[7] ? link1 : '') + (b[8] ? link2 : '') + (b[9] ? link3 : '');
            var flash = '<object type="application/x-shockwave-flash" data="http://progorodchelny.ru/advert/files/flash/' + b[4] + '" width="' + b[10] + '" height="' + b[11] + '"><param name="movie" value="http://prokazan.ru/advert/files/flash/' + b[4] + '" /><param name="quality" value="high" /><param name="wmode" value="opaque" /><param name="flashvars" value="' + vars + '"><embed type="application/x-shockwave-flash"  width="' + b[10] + '" height="' + b[11] + '" quality="high" wmode="opaque" src="http://зкщпщкщвсрудтн.ru/advert/files/flash/' + b[4] + '"' + (flashvars ? ' flashvars="' + flashvars + '"' : '') + '></embed></object>';
            obj.append(flash);
        }
        if (view == 2) {
        ...
        if (b[14] != "")
        {
            ...
            obj.append(
                $('<iframe>')
                    .attr('width', b[10])
                    .attr('height', b[11])
                    .attr('src', '/advert/files/html5/' + b[14])
                    .css({
                        'border': 0,
                    })
            );
...
/* Подгрузка новостей */
function ShowMoreInd() {
    var lastdata = $("#lastdata").val();
    $("#ShowMoreInd").html("<img src='/template/standart/loader2.gif' />");
    JsHttpRequest.query('/modules/page_mods/ShowMoreInd-JSReq.php', {'lastdata': lastdata}, function (result, errors) {
        $("#lastdata").val(result["lastdata"]);
        $("#RedColumn").append(result["text"]);
        $("#ShowMoreInd").appendTo("#RedColumn").html("<a href='javascript:void(0);' onclick='ShowMoreInd();'>Показать больше новостей</a>");
    }, true);
}
